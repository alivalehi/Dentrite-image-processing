function [minDistance,parent_mapping] = lapjv(costMat,sizx,sizy)
...
parent_mapping = zeros(size(costMat1)) ;
rowsol = rowsol(rowsol~=0);
         if(sizx>=sizy)
             for pp =1 :length(rowsol)
                 minDistance(pp) = costMat1(rowsol(pp),pp);
                 parent_mapping(pp,rowsol(pp)) = 1;
             end
         elseif(sizx<sizy)
          for pp =1 :length(rowsol)
             minDistance(pp) = costMat1(pp,rowsol(pp));
             parent_mapping(pp,rowsol(pp)) = 1;
          end
         end 
