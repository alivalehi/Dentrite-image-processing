...
% %%
% n1 = length(A(k));
% m1 = n1; % NOTE: small error still present after increasing m to 1500;
% x=datasample(A(k),1,m1);
% %___MEASUREMENT MATRIX___
% Phi = (sign(randn(m1,n1))+ones(m1,n1))/2; % micro mirror array (mma) e.g. single
% y = Phi*x;
% Theta = zeros(m1,n1);
% for ii = 1:n1
%     ii
...
% imshow(indicator);
% figure('name','Compressive sensing image reconstructions')
% subplot(1,3,1), imagesc(reshape(A,m,n)), xlabel('original'), axis image
% subplot(1,3,2), imagesc(reshape(x,m,n)), xlabel('least squares'), axis image
